module nano4k_cvbs_hdmi_diag_runtime_sweep (
    input  wire       clk_27m,
    input  wire       reset_n,
    input  wire       video_p,
    input  wire       video_n,
    ...
    // Keep OSER10 in reset until the pixel encoder is running, then release it
    // synchronously in the 126 MHz serializer domain.  This avoids both the
    // old early release and a cross-clock reset-removal path.
    reg [1:0] serializer_reset_pipe = 2'b0;
    always @(posedge clk_tmds_126m) begin
        serializer_reset_pipe <= {serializer_reset_pipe[0], pixel_resetn};
    end
    wire serializer_resetn = serializer_reset_pipe[1];

    // Exhaustively test every code 128..255 in one P&R result.  Each code lasts
    // 2^21 / 13.5 MHz = 155.3 ms (about nine captured 60 Hz frames).
    reg [20:0] sweep_counter;
    reg [6:0] sweep_index;
    always @(posedge clk_sample_13m5) begin
        if (sample_reset) begin
            sweep_counter <= 21'b0;
            sweep_index <= 7'd64; // Start at density 192.
        end else begin
    ...
    always @(posedge clk_adc_108m) begin
        if (adc_reset) begin
            ...
            sweep_meta <= sweep_index;
            sweep_sync <= sweep_meta;
        end
    end

    wire [7:0] reference_density = {1'b1, sweep_sync};
    ...
    hdmi_diag_tx hdmi (
        ...
        .reference_density_async(reference_density),
